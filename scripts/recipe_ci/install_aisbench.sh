...
cache_is_valid() {
    ...
    [[ "$(git -C "$source_directory" rev-parse HEAD 2>/dev/null)" == "$AIS_BENCH_EXPECTED_COMMIT" ]] || return 1
...
git -c http.version=HTTP/1.1 clone \
    ...
    "$AIS_BENCH_URL" "$staging_directory/source"
...
"$staging_directory/venv/bin/python" -m pip install \
    ...
    --requirement "$staging_directory/source/requirements/api.txt" \
    --requirement "$staging_directory/source/requirements/extra.txt" \
