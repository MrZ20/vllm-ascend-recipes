...
| `scenarios`           | 页面场景：每个场景含 npu/precision/deployment/case、可复用 `scripts` 和 `steps`        |
...
脚本字段只承载后续 Converter 确实需要提取的完整脚本；仅用于说明或人工执行的命令仍直接写在
...
端口等启动参数不另设重复字段，YAML 脚本中写什么，后续便提取什么。
...
| 合入前 CI | **无 GitHub Actions workflow**(`.github/` 不存在)              | PR 触发：`lint`（frontend/yaml/workflow-lint）+ `pr-recipe-verify`（单节点部署验证）+ `verify_multi_node`（多节点 Runtime 验证） |
...
4. PR Preview Build + Netlify 预览正常；多节点 Runtime workflow 验证对应的可执行 plan。
